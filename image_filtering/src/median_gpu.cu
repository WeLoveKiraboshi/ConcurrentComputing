...
void median_filter_wrapper(const cv::Mat& input, cv::Mat& output)
{
	...
	const dim3 block(BLOCKDIM,BLOCKDIM); //threads per block
    //const dim3 grid(input.cols/BLOCKDIM);
    const dim3 grid(input.cols/BLOCKDIM, input.rows/BLOCKDIM); // num blocks per grid
	//// total num of thread is calculated like
	/// block_size * grid_size = N threads

	//this example is set so as to the occupancy = 100%
    median_filter_2d_sm<<<grid,block>>>(d_input,d_output,input.cols,input.rows);
    //median_filter_2d<<<1,1>>>(d_input,d_output,input.cols,input.rows);
